...
{ 
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = RTC_ALARM_INT_PRIORITY;//抢占优先级1
...
{ 
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x15;//抢占优先级1
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x00;//子优先级2
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//使能外部中断通道
  NVIC_Init(&NVIC_InitStructure);//配置
}
...
//  u8 tbuf[40];
  (void)parameter;
  rtc_init();
//  RTC_Set_Time(19,58,56,RTC_H12_AM);	//设置时间
//  RTC_Set_Date(23,1,10,2);		//设置日期
  while(1)
  {
