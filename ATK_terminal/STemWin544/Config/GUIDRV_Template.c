...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, LCD_PIXELINDEX PixelIndex) {
    ...
    {
      g_dev.set_pixel((const char *)&PixelIndex, x, y);
    }
...
static LCD_PIXELINDEX _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      g_dev.get_pixel((char *)&PixelIndex, x, y);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    ...
    if (g_dev.draw_hline != NULL)
    {
      for (; y0 <= y1; y0++)
      {
        g_dev.draw_hline((const char *)&PixelIndex, x0, x1, y0);
      }
    }else{
      for (; y0 <= y1; y0++) {
        for (x = x0; x <= x1; x++) {
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  if (g_dev.blit_line != NULL)
  {
     g_dev.blit_line((const char *)p, x, y, xsize);
  }
  else
  {
    for (;xsize > 0; xsize--, x++, p++) {
