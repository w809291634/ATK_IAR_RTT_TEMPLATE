...
#include "serial.h"
extern unsigned char app_cpu_usage;
...
#if ( configGENERATE_RUN_TIME_STATS == 1 )
  static UBaseType_t prvListTasksRestRunTime( List_t *pxList)
	{
	volatile TCB_t *pxNextTCB, *pxFirstTCB;
	UBaseType_t uxTask = 0;
		...
		if( listCURRENT_LIST_LENGTH( pxList ) > ( UBaseType_t ) 0 )
		{
			...
			{
				listGET_OWNER_OF_NEXT_ENTRY( pxNextTCB, pxList );
        pxNextTCB->ulRunTimeCounter=0;          //复位运行时间
				uxTask++;
			} while( pxNextTCB != pxFirstTCB );
		}
		else
 ...
 void xTaskResetRunTime( void )
{
  ...
  {
    uxQueue--;
    uxTask += prvListTasksRestRunTime(&( pxReadyTasksLists[ uxQueue ] ));

  } while( uxQueue > ( UBaseType_t ) tskIDLE_PRIORITY ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */

  /* Fill in an TaskStatus_t structure with information on each
  task in the Blocked state. */
  uxTask += prvListTasksRestRunTime( ( List_t * ) pxDelayedTaskList );
  uxTask += prvListTasksRestRunTime( ( List_t * ) pxOverflowDelayedTaskList );

  #if( INCLUDE_vTaskDelete == 1 )
  {
    /* Fill in an TaskStatus_t structure with information on
    each task that has been deleted but not yet cleaned up. */
    uxTask += prvListTasksRestRunTime( &xTasksWaitingTermination );
  }
  #endif
  ...
  #if ( INCLUDE_vTaskSuspend == 1 )
  {
    /* Fill in an TaskStatus_t structure with information on
    each task in the Suspended state. */
    uxTask += prvListTasksRestRunTime(  &xSuspendedTaskList );
  }
  #endif
  CPU_RunTime = 0UL;
  ( void ) xTaskResumeAll();
...
//			ulTotalTime /= 100UL;
			...
			if( ulTotalTime > 0 )
			{
              ...
              if(memcmp(pxTaskStatusArray[ x ].pcTaskName, "IDLE", 4) == 0){
                app_cpu_usage=(unsigned char)ulStatsAsPercentage;
              }
            }
						#endif
					}
