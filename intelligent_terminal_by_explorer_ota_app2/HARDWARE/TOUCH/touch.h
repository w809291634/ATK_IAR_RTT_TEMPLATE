...
typedef struct
{
	...
	u16 y[CT_MAX_TOUCH];		//电容屏有最多5组坐标,
                          //电阻屏则用x[0],y[0]代表:此次扫描时,触屏的坐标,用x[4],y[4]存储第一次按下时的坐标. 
	u8  sta;			//笔的状态 
								//b7:按下1/松开0; 
	              //b6:0,没有按键按下;1,有按键按下. 
								//b5:保留
...
#define TDIN_H GPIO_SetBits(GPIOF,GPIO_Pin_11)	  //PF11
#define TDIN_L GPIO_ResetBits(GPIOF,GPIO_Pin_11) 
#define TCLK_H GPIO_SetBits(GPIOB,GPIO_Pin_0)	    //PB0
#define TCLK_L GPIO_ResetBits(GPIOB,GPIO_Pin_0)
#define TCS_H  GPIO_SetBits(GPIOC,GPIO_Pin_13)	 	//PC13
#define TCS_L  GPIO_ResetBits(GPIOC,GPIO_Pin_13)

#define PEN    GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_1)  //PB1
#define DOUT 	 GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_2)	//PB2
