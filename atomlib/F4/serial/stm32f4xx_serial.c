...
#define DMA_TX_STRMx            MACRO_CAT2(LL_DMA_STREAM_,DMATxSTRMn)               ///< 串口发送 dma 流     LL_DMA_STREAM_x
#define DMA_TX_IRQn             MACRO_CAT5(DMA,DMAn,_Stream,DMATxSTRMn,_IRQn)       ///< 生成 DMA 中断函数号 DMAx_Streamy_IRQn
#define DMA_TX_IRQ_HANDLER      MACRO_CAT5(DMA,DMAn,_Stream,DMATxSTRMn,_IRQHandler) ///< 生成 DMA 中断函数名 DMAx_Channely_IRQHandler
#define DMA_TX_CLEAR_FLAG_FN    MACRO_CAT2(LL_DMA_ClearFlag_TC,DMATxSTRMn)          ///< 清空 DMA 发送结束 flag

#define DMA_RX_CHx              MACRO_CAT2(LL_DMA_CHANNEL_,DMARxCHn)                ///< 串口发送 dma 通道   LL_DMA_CHANNEL_x
#define DMA_RX_STRMx            MACRO_CAT2(LL_DMA_STREAM_,DMARxSTRMn)               ///< 串口发送 dma 流     LL_DMA_STREAM_x
#define DMA_RX_IRQn             MACRO_CAT5(DMA,DMAn,_Stream,DMARxSTRMn,_IRQn)       ///< 生成 DMA 中断函数号 DMAx_Streamy_IRQn
#define DMA_RX_IRQ_HANDLER      MACRO_CAT5(DMA,DMAn,_Stream,DMARxSTRMn,_IRQHandler) ///< 生成 DMA 中断函数名 DMAx_Channely_IRQHandler
#define DMA_RX_CLEAR_FLAG_FN    MACRO_CAT2(LL_DMA_ClearFlag_TC,DMARxSTRMn)          ///< 清空 DMA 发送结束 flag
...
do{\
	LL_DMA_DisableIT_TC((h)->dma,(h)->dma_rx_stream);            \
	LL_DMA_DisableStream((h)->dma,(h)->dma_rx_stream);           \
	while(LL_DMA_IsEnabledStream((h)->dma,(h)->dma_rx_stream));  \
	(h)->dma_rx_clear((h)->dma);                                 \
	LL_DMA_SetDataLength((h)->dma,(h)->dma_rx_stream,(max));     \
	LL_DMA_SetMemoryAddress((h)->dma,(h)->dma_rx_stream,(maddr));\
	LL_DMA_EnableIT_TC((h)->dma,(h)->dma_rx_stream);             \
	LL_DMA_EnableStream((h)->dma,(h)->dma_rx_stream);            \
}while(0)
...
static void serial_init(serial_t *ttySx, uint32_t baud, uint32_t nbits, uint32_t parity, uint32_t nstop)
{
	...
	if (hal->dma_tx_enable) {
		...
		LL_DMA_DisableStream(hal->dma,hal->dma_tx_stream);//发送暂不使能
		while(LL_DMA_IsEnabledStream(hal->dma,hal->dma_tx_stream));
		
		/* USART_TX Init */ 
		LL_DMA_SetChannelSelection(hal->dma,hal->dma_tx_stream,hal->dma_tx_channel);
		LL_DMA_SetDataTransferDirection(hal->dma,hal->dma_tx_stream, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
		LL_DMA_SetStreamPriorityLevel(hal->dma,hal->dma_tx_stream, LL_DMA_PRIORITY_MEDIUM);
		LL_DMA_SetMode(hal->dma,hal->dma_tx_stream, LL_DMA_MODE_NORMAL);
		LL_DMA_SetPeriphIncMode(hal->dma,hal->dma_tx_stream, LL_DMA_PERIPH_NOINCREMENT);
		LL_DMA_SetMemoryIncMode(hal->dma,hal->dma_tx_stream, LL_DMA_MEMORY_INCREMENT);
		LL_DMA_SetPeriphSize(hal->dma,hal->dma_tx_stream, LL_DMA_PDATAALIGN_BYTE);
		LL_DMA_SetMemorySize(hal->dma,hal->dma_tx_stream, LL_DMA_MDATAALIGN_BYTE);
		LL_DMA_SetPeriphAddress(hal->dma,hal->dma_tx_stream,LL_USART_DMA_GetRegAddr(hal->uart));
		LL_DMA_DisableFifoMode(hal->dma,hal->dma_tx_stream);
		...
		LL_DMA_EnableIT_TC(hal->dma,hal->dma_tx_stream);
	}
	...
	if (hal->dma_rx_enable) {
		...
		LL_DMA_DisableStream(hal->dma,hal->dma_rx_stream);//发送暂不使能
		while(LL_DMA_IsEnabledStream(hal->dma,hal->dma_rx_stream));

		/* USART_RX Init */
		LL_DMA_SetChannelSelection(hal->dma,hal->dma_rx_stream,hal->dma_rx_channel);
		LL_DMA_SetDataTransferDirection(hal->dma,hal->dma_rx_stream, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
		LL_DMA_SetStreamPriorityLevel(hal->dma,hal->dma_rx_stream, LL_DMA_PRIORITY_MEDIUM);
		LL_DMA_SetMode(hal->dma,hal->dma_rx_stream, LL_DMA_MODE_NORMAL);
		LL_DMA_SetPeriphIncMode(hal->dma,hal->dma_rx_stream, LL_DMA_PERIPH_NOINCREMENT);
		LL_DMA_SetMemoryIncMode(hal->dma,hal->dma_rx_stream, LL_DMA_MEMORY_INCREMENT);
		LL_DMA_SetPeriphSize(hal->dma,hal->dma_rx_stream, LL_DMA_PDATAALIGN_BYTE);
		LL_DMA_SetMemorySize(hal->dma,hal->dma_rx_stream, LL_DMA_MDATAALIGN_BYTE);
		LL_DMA_SetPeriphAddress(hal->dma,hal->dma_rx_stream,LL_USART_DMA_GetRegAddr(hal->uart));
		LL_DMA_DisableFifoMode(hal->dma,hal->dma_rx_stream);

		NVIC_SetPriority(hal->dma_rx_irq, 
			NVIC_EncodePriority(NVIC_GetPriorityGrouping(),hal->dma_priority, 0));
		NVIC_EnableIRQ(hal->dma_rx_irq);
		LL_DMA_EnableIT_TC(hal->dma,hal->dma_rx_stream);
...
static void serial_deinit(serial_t *ttySx)
{
	...
	if (hal->dma_rx_enable) {
		...
		LL_DMA_DisableIT_TC(hal->dma,hal->dma_rx_stream);
		NVIC_DisableIRQ(hal->dma_rx_irq);
	}

	if (hal->dma_tx_enable) {
		LL_USART_DisableDMAReq_TX(hal->uart);
		LL_DMA_DisableIT_TC(hal->dma,hal->dma_tx_stream);
...
static void usart_irq(serial_t *ttySx)
{
	...
	if (LL_USART_IsActiveFlag_IDLE(hal->uart)) { //空闲中断
		...
		if (hal->dma_rx_enable){
			unsigned short pktlen = ttySx->rxmax - LL_DMA_GetDataLength(hal->dma,hal->dma_rx_stream);//得到当前包的长度
			if (pktlen) {
	...
	if (LL_USART_IsActiveFlag_TC(hal->uart)) { // 发送中断
		...
		if (hal->dma_tx_enable) {              // 如果是非 dma 方式发送
			LL_DMA_DisableStream(hal->dma,hal->dma_tx_stream); //停止 DMA
			LL_USART_DisableIT_TC(hal->uart);
...
static inline void dma_send_pkt(serial_t * ttySx)
{
	...
	ttySx->txsize = 0;
	while(LL_DMA_IsEnabledStream(hal->dma,hal->dma_tx_stream));
	LL_DMA_SetDataLength(hal->dma,hal->dma_tx_stream,pkt_size);
	LL_DMA_SetMemoryAddress(hal->dma,hal->dma_tx_stream,
		(uint32_t)(&ttySx->txbuf[pkt_head]));
	LL_DMA_EnableStream(hal->dma,hal->dma_tx_stream); 
}
...
int serial_write(serial_t *ttySx , const void * data , int datalen , int block ) 
{
	...
	if (remain && datalen) {//当前发送缓存有空间
		...
		if (hal->dma_tx_enable) {                       // dma 模式下发送
			...
			if (!LL_DMA_IsEnabledStream(hal->dma,hal->dma_tx_stream))//开始发送
				dma_send_pkt(ttySx);
			USART_TX_UNLOCK(hal);
		}
		else  {
