...
space = (x) *6 + 3;
                         ...
                         ____________________ 576 ____________________
             ________ 452 ________                           ________ 874 ________
       __ 344 __               __.539 __               __ 856.__               __.978 __
    200.        419.        504.        562         784        .872         974.        996
 [ ]   258   [ ]   424   [ ]   526   [ ]   [ ]   [ ]   [ ]   871   [ ]   [ ]   975   [ ]   [ ]
...
void print_mytree(const struct avl_root *root)
{
	...
	for (int deepc = 1; deepc <= mytreedeep; ++deepc)
	{
		...
		linec = (emptyc / 2 > 0) ? (emptyc * 3 - 5) : 0;
		//linec = (emptyc * 6 - 5) ;

		if (linec > 0)
			spacec = (emptyc > 0) ? (emptyc * 6 + 4) : 1; //空格数
		else
			spacec = (emptyc > 0) ? 6 : 0;
		...
		for (widthc = 1; widthc <= widthmax; ++widthc){
			...
			if ((node = get_tree_node(root, deepc, widthc))){
				...
				switch (avl_scale(node)){
					case AVL_BALANCED  :printf(" %04d ", myavldata->myvalue); break;
					case AVL_TILT_LEFT :printf(".%04d ", myavldata->myvalue); break;
					case AVL_TILT_RIGHT:printf(" %04d.", myavldata->myvalue); break;
				}
			}
			else {
				printf(" [  ] ");
