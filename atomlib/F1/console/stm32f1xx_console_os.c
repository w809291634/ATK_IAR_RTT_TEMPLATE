...
/*
  * for freertos;
	STM32CUBEMX set: CONFIGURE_TIMER_FOR_RUN_TIME_STATS , USE_TRACE_FACILITY ,USE_STATS_FORMATTING_FUNCTIONS
...
void task_list(void * arg)
{
	...
	#else 
		printk("please set\r\nconfigUSE_STATS_FORMATTING_FUNCTIONS & configUSE_TRACE_FACILITY as 1.\r\n");
	#endif 
}

void task_runtime(void * arg)
{
	#if (portCONFIGURE_TIMER_FOR_RUN_TIME_STATS && configUSE_TRACE_FACILITY) 
		static const char title[] = "\r\nthread\t\ttime\t\t%CPU";
		char * tasklistbuf = (char *)pvPortMalloc(2048);
		...
		vPortFree(tasklistbuf);
	#else 
		printk("please set\r\nportCONFIGURE_TIMER_FOR_RUN_TIME_STATS & configUSE_TRACE_FACILITY as 1.\r\n");
	#endif 
}
...
/**
  * @brief    命令行响应函数，进行 iap 升级，从串口接收数据包
  * @param    arg  : 命令行内存指针
  * @return   void
*/
...
/**
  * @brief    获取 syscfg 信息，由 shell_into_edit 调用
  * @param
...
/**
  * @brief    更新 syscfg 信息，由 shell_into_edit 调用
  * @param
  * @return   void
*/
