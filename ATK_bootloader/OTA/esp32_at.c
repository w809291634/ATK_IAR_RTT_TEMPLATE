...
static int esp32_connect_server_handle(void)
{
  ...
  switch(send_cmd_count){
    ...
    case 4:{
      ...
      esp32_send_cmd(current_cmd,strlen(current_cmd));
      softTimer_start(ESP32_TIMEOUT_TIMER_ID,17000);
    }break;
    case 5:{
      /* AT+CIPSEND 进入透传模式 */
      send_cmd_count=send_cmd_count%ARRAY_SIZE(cmd_list);
...
void esp32_reset(void)
{
  ...
  RCC_AHB1PeriphClockCmd(ESP32_RESET_RCC, ENABLE);  //使能GPIOF时钟

  //GPIOF9,F10初始化设置
  GPIO_InitStructure.GPIO_Pin = ESP32_RESET_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;     //普通输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(ESP32_RESET_GPIO, &GPIO_InitStructure); //初始化GPIO
	
	GPIO_ResetBits(ESP32_RESET_GPIO,ESP32_RESET_PIN);   
  hw_ms_delay(100);
  GPIO_SetBits(ESP32_RESET_GPIO,ESP32_RESET_PIN); 
}
...
{
  softTimer_create(ESP32_CONNECT_ID,MODE_ONE_SHOT,_esp32_connect_start);
  
  if(esp32_link){
    // 退出透传模式,再次连接
    esp32_exit_transparent();
